...
function sendHtml(
  res: Response,
  { ok, returnTo = "/", id, message }: { ok: boolean; returnTo?: string; id?: string; message?: string }
) {
...
<p><a href="${returnTo}">Continue</a></p><script>location.replace(${JSON.stringify(returnTo)});</script>`);
}

// --- Admin Home (MUST exist so Preview works) ---
app.get("/", (req, res) => {
...
app.all("/reviews/ping", (_req, res) => res.status(200).type("text/plain").send("ok from /reviews/ping"));

// --- Helper: product id → GID ---
...
app.all("/reviews/submit", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // ---- TOKEN: DEV shortcut (no OAuth yet) ----
    // Put a Custom App on your DEV STORE only and copy its Admin API token.
    // Set Vercel env: DEV_SHOP_DOMAIN, DEV_ADMIN_TOKEN
    let accessToken: string | undefined;
    if (process.env.DEV_SHOP_DOMAIN && process.env.DEV_ADMIN_TOKEN && shop === process.env.DEV_SHOP_DOMAIN) {
      accessToken = process.env.DEV_ADMIN_TOKEN;
    } else {
      // Fallback: Prisma session (for when you later add OAuth)
      const { PrismaClient } = await import("@prisma/client");
      const prisma = new PrismaClient();
      const session = await prisma.session.findFirst({ where: { shop, isOnline: false } });
      accessToken = session?.accessToken;
    }
    if (!accessToken) return sendHtml(res, { ok: false, returnTo, message: "App not authorized for this shop" });
    ...
    if (!Number.isInteger(rating) || rating < 1 || rating > 5) return sendHtml(res, { ok: false, returnTo, message: "Rating must be 1..5" });

    // Create review metaobject (type "review" must exist)
    ...
    let json: any; try { json = JSON.parse(raw); } catch { return sendHtml(res, { ok:false, returnTo, message:"API returned non-JSON" }); }
    const errs = json?.data?.metaobjectCreate?.userErrors ?? [];
    if (errs.length) return sendHtml(res, { ok:false, returnTo, message:"Validation error" });
